...
dataset_dir = root_dir / 'data_thermomechanic'
sys.path.append(str(root_dir))
tmp_results_dir = root_dir / '.tmp_results'
tmp_results_dir.mkdir(parents=True, exist_ok=True)
t3_results_dir = tmp_results_dir / 't3_plane_stress_problem'
t3_results_dir.mkdir(parents=True, exist_ok=True)
...
from utils.normalization import norm_array_0_1
import utils.errors_np as errors_np

torch.manual_seed(1234)
np.random.seed(1234)

from utils.data_structure import gradients, to_numpy, to_tensor
...
class Model(nn.Module):
    ...
    def closure1(self):
        ...
        # self.ls = ls7 + ls8 + ls9
        # self.ls = ls1 + ls2 + ls6 + ls7 + ls8 + ls9
        self.ls = ls1 + ls2 + ls3 + ls4 + ls5 + ls6 + ls7 + ls8 + ls9
        # self.ls = ls1 + ls2 + ls3 + ls4 + ls5 + ls6
        # self.ls = ls3 + ls4 + ls5 + ls6
        # self.ls = ls1 + ls2 + ls6
        self.ls.backward()
    ...
    def train(self):
        device = torch.device("cuda:0")
        self.__history_generate_object_number = 12
        ...
        for i in range(5000):
            self.optimizer.step(self.closure1)
        for i in range(5000):
...
if __name__ == "__main__":
    ...
    #     data = np.loadtxt(f)
    with open(dataset_dir / 'shape6.txt', 'r') as f:
        data = np.loadtxt(f)
    # data = np.load("./data/shape5_med2.npy")  # load incomplete dataset
    ...
    # normalization: forward & backward pass
    ...
    norm_params = np.array(norm_params, dtype=float)  # norm_params are recorded to normalize field variables & PDEs
    ...
    """shape6
        model1    approximate strain only
        model2    approximate displacement only
        model3    approximate displacement and strain
        model4    approximate displacement, strain and stress
        model5    approximate displacement and stress
        model6    approximate stress only
        model7    classical PINN implementation (displacement + strain + stress + balance equation)
        (unsaved_model)  train closure2 and 4 for 5000iters based on model4:  approximate displacement + strain + stress + Multi-task re-training

        model8    use the `shape6_med` dataset. Train closure1 for 5000 iters, and train closure2 (removing u, v) for 5000(lr=0.00005) + 10000(0.00001) + 5000(0.000005) iters
        Taking model8 as the base pre-trained model. Re-train based on it will acquire significantlly better MSE loss!
        model8_1    train closure1 for 5000(0.00001) + 5000(0.000005) iters
        model8_2    train closure1 5000(0.00001) + closure2 2000(0.000005) + closure1 5000(0.000005) iters
        
        The displacement fields only require simple approximation to gain highly-consistent approximation.
        The stress and strain fields are influenced by the base pre-trained model. Try to replace the dataset used for pre-training,
        and gain a better pre-trained model.

        model9    the `shape6` dataset. closure 1:5000, 2:5000(0.00015) + 5000(0.00005) + 10000(0.00001)
        model9_1    1: 10000(0.00004) + 10000(0.00001)
        model9_2    0_1: 4000(0.00004), 1: 2000(0.00004) + 5000(0.00001) ★★
        model9_3    0_1: 4500(0.00004), 1: 2000(0.00004) + 6500(0.00001)
        
       shape10
        model0    approximate displacement only
        model1    approximate displacement, strain and stress
        model2    classical PINN implementation
    """
    # # --------------------------------- MODEL & TRAIN ---------------------------------
    # pinn_model = Model(in_data, bd_data)
    # pinn_model.train()
    # torch.save(pinn_model.net_u_x, t3_results_dir / 'shape6_model9_u_x.pt')
    # torch.save(pinn_model.net_u_y, t3_results_dir / 'shape6_model9_u_y.pt')
    # torch.save(pinn_model.net_e_z, t3_results_dir / 'shape6_model9_e_z.pt')
    # # 记录过程输出 loss
    # history = pinn_model.get_history_loss()
    # history = to_numpy(history)
    # np.save(t3_results_dir / 'shape6_history9.npy', history)


    # # ------------------------------ MODEL RE-LOAD ------------------------------
    # del pinn_model
    # pinn_model = Model(in_data, bd_data)
    # pinn_model.net_u_x = torch.load(t3_results_dir / 'shape6_model9_u_x.pt')
    # pinn_model.net_u_y = torch.load(t3_results_dir / 'shape6_model9_u_y.pt')
    # pinn_model.net_e_z = torch.load(t3_results_dir / 'shape6_model9_e_z.pt')
    ...
    # torch.save(pinn_model.net_u_x, t3_results_dir / 'shape6_model9_2_u_x.pt')
    # torch.save(pinn_model.net_u_y, t3_results_dir / 'shape6_model9_2_u_y.pt')
    # torch.save(pinn_model.net_e_z, t3_results_dir / 'shape6_model9_2_e_z.pt')
    
    # # 记录 re-training 输出 loss
    # history = pinn_model.get_history_loss()
    # history = to_numpy(history)
    # np.save(t3_results_dir / 'shape6_history9_2.npy', history)
    
    # ------------------------------ MODEL RE-LOAD ------------------------------
    # del pinn_model
    pinn_model = Model(in_data, bd_data)
    pinn_model.net_u_x = torch.load(t3_results_dir / 'shape6_model9_2_u_x.pt')
    pinn_model.net_u_y = torch.load(t3_results_dir / 'shape6_model9_2_u_y.pt')
    pinn_model.net_e_z = torch.load(t3_results_dir / 'shape6_model9_2_e_z.pt')
    ...
    #     data = np.loadtxt(f)
    with open(dataset_dir / 'shape6_22.txt', 'r') as f:
        data = np.loadtxt(f)
    print(data.shape)
    ...
    norm_v = norm_array_0_1(data[:, 10])
    norm_e_z = norm_array_0_1(data[:, 11])
    # 由于后面还要用这些数据恢复归一化之前的状态，所以这里用了一个 class 的设计来记录倍率
    data[:, 3], aa = norm_s_x.norm_forward()
    data[:, 5], bb = norm_s_y.norm_forward()
    data[:, 4], cc = norm_s_xy.norm_forward()
    data[:, 6], dd = norm_e_x.norm_forward()
    ...
    """输出为：
        function_s : s_x, s_y, s_xy, s_x_x, s_y_y, s_xy_x, s_xy_y
        function_e : u_x, u_y, e_x, e_y, e_xy
    """
    # u_x_pred, u_y_pred, e_x_pred, e_y_pred, e_xy_pred, _ = pinn_model.function(x_test, y_test)
    t1 = time.time()
    ...
    fig.suptitle(r'$\sigma, \varepsilon$ : FEM(left); Prediction(right)')
    plt.subplots_adjust(top=0.93)
    
    ax[0, 0].scatter(x, y, c=s_x.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(s_x), vmax=max(s_x), alpha=0.9, s=1)
    ax[0, 1].scatter(x, y, c=s_x_pred.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(s_x), vmax=max(s_x), alpha=0.9, s=1)
    ax[1, 0].scatter(x, y, c=s_y.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(s_y), vmax=max(s_y), alpha=0.9, s=1)
    ax[1, 1].scatter(x, y, c=s_y_pred.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(s_y), vmax=max(s_y), alpha=0.9, s=1)
    ax[2, 0].scatter(x, y, c=s_xy.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(s_xy), vmax=max(s_xy), alpha=0.9, s=1)
    ax[2, 1].scatter(x, y, c=s_xy_pred.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(s_xy), vmax=max(s_xy), alpha=0.9, s=1)
    ax[0, 0].set_title(r"$\sigma_{x,fem}$")
    ax[0, 1].set_title(r"$\sigma_{x,pred}$")
    ax[1, 0].set_title(r"$\sigma_{y,fem}$")
    ax[1, 1].set_title(r"$\sigma_{y,pred}$")
    ax[2, 0].set_title(r"$\tau_{xy,fem}$")
    ax[2, 1].set_title(r"$\tau_{xy,pred}$")
    ax[0, 2].scatter(x, y, c=e_x.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(e_x), vmax=max(e_x), alpha=0.9, s=1)
    ax[0, 3].scatter(x, y, c=e_x_pred.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(e_x), vmax=max(e_x), alpha=0.9, s=1)
    ax[1, 2].scatter(x, y, c=e_y.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(e_y), vmax=max(e_y), alpha=0.9, s=1)
    ax[1, 3].scatter(x, y, c=e_y_pred.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(e_y), vmax=max(e_y), alpha=0.9, s=1)
    ax[2, 2].scatter(x, y, c=e_xy.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(e_xy), vmax=max(e_xy), alpha=0.9, s=1)
    ax[2, 3].scatter(x, y, c=e_xy_pred.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(e_xy), vmax=max(e_xy), alpha=0.9, s=1)
    ax[0, 2].set_title(r"$\varepsilon_{x,fem}$")
    ax[0, 3].set_title(r"$\varepsilon_{x,pred}$")
    ax[1, 2].set_title(r"$\varepsilon_{y,fem}$")
    ax[1, 3].set_title(r"$\varepsilon_{y,pred}$")
    ax[2, 2].set_title(r"$\gamma_{xy,fem}$")
    ax[2, 3].set_title(r"$\gamma_{xy,pred}$")
    ax[3, 0].scatter(x, y, c=u.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(u), vmax=max(u), alpha=0.9, s=1)
    ax[3, 1].scatter(x, y, c=u_x_pred.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(u), vmax=max(u), alpha=0.9, s=1)
    ax[3, 2].scatter(x, y, c=v.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(v), vmax=max(v), alpha=0.9, s=1)
    ax[3, 3].scatter(x, y, c=u_y_pred.reshape(x.shape[0]), cmap=plt.cm.jet, vmin=min(v), vmax=max(v), alpha=0.9, s=1)
    ax[3, 0].set_title(r"$u_{x,fem}$")
    ax[3, 1].set_title(r"$u_{x,pred}$")
    ax[3, 2].set_title(r"$u_{y,fem}$")
    ...
    plt.savefig(dataset_dir / 'fields.png', dpi=300)

    # ------------------------------------ TEST ERRORS ------------------------------------
    # err_s_x = np.abs( s_x.reshape(x.shape[0]) - s_x_pred.reshape(x.shape[0]) )
    # err_s_y = np.abs( s_y.reshape(x.shape[0]) - s_y_pred.reshape(x.shape[0]) )
    # err_s_xy = np.abs( s_xy.reshape(x.shape[0]) - s_xy_pred.reshape(x.shape[0]) )
    # err_e_x = np.abs( e_x.reshape(x.shape[0]) - e_x_pred.reshape(x.shape[0]) )
    # err_e_y = np.abs( e_y.reshape(x.shape[0]) - e_y_pred.reshape(x.shape[0]) )
    # err_e_xy = np.abs( e_xy.reshape(x.shape[0]) - e_xy_pred.reshape(x.shape[0]) )
    ...
    # fig1.suptitle(r'$\sigma, \varepsilon$ : Generated Errors')
    # plt.subplots_adjust(top=0.85)
    
    # ax1[0, 0].scatter(x, y, c=err_s_x, cmap=plt.cm.jet, vmin=min(err_s_x), vmax=max(err_s_x), alpha=0.8, s=1)
    # ax1[1, 0].scatter(x, y, c=err_s_y, cmap=plt.cm.jet, vmin=min(err_s_y), vmax=max(err_s_y), alpha=0.8, s=1)
    # ax1[2, 0].scatter(x, y, c=err_s_xy, cmap=plt.cm.jet, vmin=min(err_s_xy), vmax=max(err_s_xy), alpha=0.8, s=1)
    # ax1[0, 1].scatter(x, y, c=err_e_x, cmap=plt.cm.jet, vmin=min(err_e_x), vmax=max(err_e_x), alpha=0.8, s=1)
    # ax1[1, 1].scatter(x, y, c=err_e_y, cmap=plt.cm.jet, vmin=min(err_e_y), vmax=max(err_e_y), alpha=0.8, s=1)
    # ax1[2, 1].scatter(x, y, c=err_e_xy, cmap=plt.cm.jet, vmin=min(err_e_xy), vmax=max(err_e_xy), alpha=0.8, s=1)
    # ax1[0, 0].set_title(r"$\sigma_x$")
    # ax1[1, 0].set_title(r"$\sigma_y$")
    # ax1[2, 0].set_title(r"$\sigma_{xy}$")
    # ax1[0, 1].set_title(r"$\varepsilon_x$")
    # ax1[1, 1].set_title(r"$\varepsilon_y$")
    # ax1[2, 1].set_title(r"$\varepsilon_{xy}$")
    
    # fig1.colorbar(plt.cm.ScalarMappable(mpl.colors.Normalize(vmin=min(err_s_x), vmax=max(err_s_x), clip=True), cmap=plt.cm.jet), ax=ax1[0, 0])
    # fig1.colorbar(plt.cm.ScalarMappable(mpl.colors.Normalize(vmin=min(err_s_y), vmax=max(err_s_y), clip=True), cmap=plt.cm.jet), ax=ax1[1, 0])
    # fig1.colorbar(plt.cm.ScalarMappable(mpl.colors.Normalize(vmin=min(err_s_xy), vmax=max(err_s_xy), clip=True), cmap=plt.cm.jet), ax=ax1[2, 0])
    # fig1.colorbar(plt.cm.ScalarMappable(mpl.colors.Normalize(vmin=min(err_e_x), vmax=max(err_e_x), clip=True), cmap=plt.cm.jet), ax=ax1[0, 1])
    # fig1.colorbar(plt.cm.ScalarMappable(mpl.colors.Normalize(vmin=min(err_e_y), vmax=max(err_e_y), clip=True), cmap=plt.cm.jet), ax=ax1[1, 1])
    # fig1.colorbar(plt.cm.ScalarMappable(mpl.colors.Normalize(vmin=min(err_e_xy), vmax=max(err_e_xy), clip=True), cmap=plt.cm.jet), ax=ax1[2, 1])
    ...
    #     z = s_y_pred.reshape(x.shape[0]),
    #     mode = "markers",
    #     marker = dict(color='#ffa502', size=3),
    #     name="predict"
    # )
    # trace2 = go.Scatter3d(
    #     x = x.reshape(x.shape[0]),
    #     y = y.reshape(x.shape[0]),
    #     z = s_y.reshape(x.shape[0]),
    #     mode = "markers",
    #     marker = dict(color='#22a6b3', size=3),
    #     name="fem"
    # )
    ...
    #     y = y.reshape(x.shape[0]),
    #     mode = "markers",
    #     marker = dict(color=s_y_pred.reshape(x.shape[0]), size=5),
    #     # name="s_y predict"
    # )
    ...
    data = data[data[:, 1] < 0.24, :]
    data = data[data[:, 1] > 0.16, :]
    ax.scatter(data[:,0], data[:, 1], s=3, c="black")
    plt.savefig(dataset_dir / 'sample.png', dpi=300)
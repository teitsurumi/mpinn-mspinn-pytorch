...
t3_results_dir = tmp_results_dir / 't3_plane_stress_problem_energy'
...
class Model(nn.Module):
    def __init__(
        ...
        # Delaunay 积分
        # data
        delaunay_data = np.load(dataset_dir / 'shape6_delaunay2.npz')
    ...
    def closure1(self):
        ...
        if not self.iter1 % 200:
            ...
            # print(e_z.reshape(self.interior_num))
            # print(self.norm_params[8][1])
            # print(self.norm_params[8][0])
            # print(ls6)
            self.time1 = time.time()
        self.iter1 += 1
        return self.ls
...
if __name__ == "__main__":
    ...
    # # # # # # # # # # # --------------------------------- ALREADY TRAINED IN `t3_plane_stress_problem.py` ---------------------------------
    ...
    # ------------------------------ RE-TRAIN ------------------------------
    pinn_model.re_train()
    torch.save(pinn_model.net_u_x, t3_results_dir / 'shape6_model9_e6_u_x.pt')
    torch.save(pinn_model.net_u_y, t3_results_dir / 'shape6_model9_e6_u_y.pt')
    torch.save(pinn_model.net_e_z, t3_results_dir / 'shape6_model9_e6_e_z.pt')
    history = pinn_model.get_history_loss()
    history = to_numpy(history)
    np.save(t3_results_dir / 'shape6_history9_e6.npy', history)
    
    # ------------------------------ MODEL RE-GENERATE ------------------------------
    del pinn_model
    pinn_model = Model(in_data, bd_data)
    pinn_model.net_u_x = torch.load(t3_results_dir / 'shape6_model9_e6_u_x.pt')
    pinn_model.net_u_y = torch.load(t3_results_dir / 'shape6_model9_e6_u_y.pt')
    pinn_model.net_e_z = torch.load(t3_results_dir / 'shape6_model9_e6_e_z.pt')
    ...
    # -------------------------------------- TEST --------------------------------------
    with open(dataset_dir / 'shape6_full_full.txt', 'r') as f:
        data = np.loadtxt(f)
    norm_s_x  = norm_array_0_1(data[:, 3])
    norm_s_y  = norm_array_0_1(data[:, 5])
    norm_s_xy = norm_array_0_1(data[:, 4])
    ...
    plt.savefig(t3_results_dir / 'fields.png', dpi=300)

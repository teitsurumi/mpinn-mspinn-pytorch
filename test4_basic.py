...
if __name__ == "__main__":
    device = torch.device("cuda:0")
    # with open("data_thermoelastic/shape4node4.txt", 'r') as f:
    #     data = np.loadtxt(f)
    with open("data_thermoelastic/shape6.txt", 'r') as f:
        data = np.loadtxt(f)

    # load the incomplete dataset
    # data = np.load("data_thermoelastic/shape5_med2.npy")
    ...
    # use a Class here to recover these data after prediction
    ...
    """shape6
        model1    displacement function method (solely approximate the strain to investigate the convergence of other loss terms)
        model2    displacement function method (solely approximate the displacement to investigate the convergence of other loss terms)
        model3    displacement function method (approximate the strain & displacement to investigate the convergence of other loss terms)
        model4    displacement function method (approximate the strain & displacement & stress to investigate the convergence of other loss terms)
        model5    displacement function method (approximate the displacement & stress to investigate the convergence of other loss terms)
        model6    displacement function method (solely approximate the stress to investigate the convergence of other loss terms)
        
        model7    Original PINN
        model unsaved
                  re-load model4 and re-train closure2 & closure4 for 5000 iters
                  approximate strain & displacement & stress
                  Multi-stage re-training for several times

        model8    shape6_med.txt:  closure1: 5000 it, closure2: 5000(lr=0.00005) + 10000(0.00001) + 5000(0.000005)
        model8_1  closure1: 5000(0.00001) + 5000(0.000005)
        model8_2  closure1 5000(0.00001) + closure2 2000(0.000005) + closure1 5000(0.000005)
        
        
        model9    shape6.txt:  1:5000, 2:5000(0.00015) + 5000(0.00005) + 10000(0.00001)
        model9_1  1: 10000(0.00004) + 10000(0.00001)
        model9_2  0_1: 4000(0.00004), 1: 2000(0.00004) + 5000(0.00001) (best)
        model9_3  0_1: 4500(0.00004), 1: 2000(0.00004) + 6500(0.00001)
        
       shape10
        model0    displacement only
        model1    strain & displacement & stress
        model2    Original PINN
    """
    # --------------------------------- MODEL & TRAIN ---------------------------------
    pinn_model = Model(in_data, bd_data)
    pinn_model.train()
    # torch.save(pinn_model.net_u_x, './models_v4_s/shape6_model1_u_x.pt')
    # torch.save(pinn_model.net_u_y, './models_v4_s/shape6_model1_u_y.pt')
    # torch.save(pinn_model.net_e_z, './models_v4_s/shape6_model1_e_z.pt')
    # history = pinn_model.get_history_loss()
    # history = to_numpy(history)
    # np.save("./models_v4/shape6_history8.npy", history)
    ...
    # del pinn_model
    # pinn_model = Model(in_data, bd_data)
    # pinn_model.net_u_x = torch.load('./models_v4/shape6_model9_2_u_x.pt')
    # pinn_model.net_u_y = torch.load('./models_v4/shape6_model9_2_u_y.pt')
    # pinn_model.net_e_z = torch.load('./models_v4/shape6_model9_2_e_z.pt')
    ...
    # torch.save(pinn_model.net_u_x, './models_v4/shape6_model9_3_u_x.pt')
    # torch.save(pinn_model.net_u_y, './models_v4/shape6_model9_3_u_y.pt')
    # torch.save(pinn_model.net_e_z, './models_v4/shape6_model9_3_e_z.pt')
    
    # history = pinn_model.get_history_loss()
    # history = to_numpy(history)
    # np.save("./models_v4/shape6_history9_2.npy", history)

    # -------------------------------------- TEST --------------------------------------
    # with open("data_thermoelastic/shape4node4_med.txt", 'r') as f:
    #     data = np.loadtxt(f)
    # with open("data_thermoelastic/shape6_22.txt", 'r') as f:
    #     data = np.loadtxt(f)
    with open("data_thermoelastic/shape6_full_full.txt", 'r') as f:
        data = np.loadtxt(f)
    print(data.shape)
